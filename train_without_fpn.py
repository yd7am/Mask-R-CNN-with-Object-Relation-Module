...
import transforms
from model.rpn_function import AnchorsGenerator
from model.mask_rcnn import MaskRCNN
from datasets_coco import DatasetsCOCO
...
def main(args):
    ...
    det_results_file = f"./result/det_results{now}.txt"
    seg_results_file = f"./result/seg_results{now}.txt"
    ...
    train_dataset = DatasetsCOCO(data_root, "train", data_transform["train"])
    ...
    # 注意这里的collate_fn是自定义的，因为读取的数据包括image和targets，不能直接使用默认的方法合成batch(大小不一致)
    ...
    val_dataset = DatasetsCOCO(data_root, "val", data_transform["val"])
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录
    parser.add_argument('--data-path', default='./PN_Instance_COCO_datasets', help='dataset')
    # 检测目标类别数(不包含背景)
