...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    labels = torch.cat(labels, dim=0)  # [batch_size, 512] -> [batch_size*512]
    regression_targets = torch.cat(regression_targets, dim=0)  # [batch_size, 512, 4] -> [batch_size*512, 4]
    # 样本不平衡
    weight = torch.tensor([len(torch.where(torch.eq(labels, label))[0]) for label in range(class_logits.shape[1])]).to(labels)
    weight = (labels.shape[0] - weight) / labels.shape[0]

    # 计算类别损失信息 class_logits:[512*batch_size, num_classes+1]
    # class_logits：softmax？labels：one-hot？
    ...
    box_regression = box_regression.reshape(N, -1, 4)  # [batch_size*512, num_classes*4] -> [batch_size*512, num_classes, 4]
...
def maskrcnn_inference(x, labels):
    ...
    # x: [all_nums_proposals, num_classes, 28, 28]
    mask_prob = x.sigmoid()
    ...
    # mask是根据cls_logits(labels)选取的
    mask_prob = mask_prob[index, labels][:, None]  # [all_nums_proposals, 1, 28, 28]
    # 最后再按照每张图片中的masks个数分离开
    mask_prob = mask_prob.split(boxes_per_image, dim=0)  # [nums_mask_per_image, 1, 28, 28]*batch_size
...
def maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels, mask_matched_idxs):
    ...
    Args:
        mask_logits: [all_nums_pos, num_classes, 28, 28]
        proposals: [nums_pos, 4]*batch_size
        gt_masks: [nums_instance, resized_H, resized_W]*batch_size
        gt_labels: [nums_instance]*batch_size
        mask_matched_idxs: [nums_pos]*batch_size
        nums_pos >= nums_instance 因为gt也被当做正样本
    ...
    labels = torch.cat(labels, dim=0)  # [nums_pos*batch_size]
    ...
    # 没有经过softmax，只计算其中一类mask_logit与gt的损失
...
class RoIHeads(torch.nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            if gt_boxes_in_image.numel() == 0:  # 该张图像中没有gt框，为背景
                ...
                # iou < low_threshold索引值为 -1， low_threshold <= iou < high_threshold索引值为 -2（Low_T==High_T）
                # [2000+nums_gt_boxes]
    ...
    def subsample(self, labels):
        ...
        # BalancedPositiveNegativeSampler->掩码
    ...
    def select_training_samples(self,
        ...
        gt_boxes = [t["boxes"].to(dtype) for t in targets]  # [num_coco_targets, 4]*batch_size
        gt_labels = [t["labels"] for t in targets]  # [num_coco_targets]*batch_size

        # append ground-truth bboxes to proposal
        # 将gt_boxes拼接到proposal后面 增加正样本个数
        ...
        # 遍历每张图像2000+nums_gt->512
        for img_id in range(num_images):
            # 获取每张图像的正负样本索引
            img_sampled_inds = sampled_inds[img_id]
            # 获取对应正负样本的proposals信息  2000+nums_gt->512
        ...
        # bbox_reg_weights = (10., 10., 5., 5.) [dx,dy,dw,dh]
    ...
    def postprocess_detections(self,
        ...
        # 获取每张图像的预测bbox数量 1000
        boxes_per_image = [boxes_in_image.shape[0] for boxes_in_image in proposals]
        # 根据proposal以及预测的回归参数计算出最终bbox坐标
        # proposals:1000, box_regression:[1000*batch_size, 4*(num_classes+1)]
        pred_boxes = self.box_coder.decode(box_regression, proposals)  # [1000, num_classes, 4]
        ...
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
            ...
            # 移除索引为0的所有信息（0代表背景）
            boxes = boxes[:, 1:]  # [1000, num_classes-1, 4]
            scores = scores[:, 1:]  # [1000, num_classes-1]
            labels = labels[:, 1:]  # [1000, num_classes-1]
            ...
            # 移除低概率目标，self.scores_thresh=0.05  1/20
            # 
            ...
            # 这里与rpn对每个特征图nms不同，这里对每个类别nms
            keep = box_ops.batched_nms(boxes, scores, labels, self.nms_thresh)

            # keep only topk scoring predictions
            # 获取scores排在前topk个预测目标 100
    ...
    def forward(self,
        ...
        """
        Arguments:
            features (List[Tensor])  {OrderedDict:5}
        ...
        if self.training:
            # 划分正负样本，统计对应gt的标签以及边界框回归信息
            proposals, matched_idxs, labels, regression_targets = self.select_training_samples(proposals, targets)  # batch中每副图像2000->512
        else:  # 1000
            labels = None
        ...
        # train:[512*batch_size, channel of features, 7, 7]
        # predict:[1000*batch_size, channel of features, 7, 7]
        ...
        # 接着分别预测目标类别和边界框回归参数
        # class_logits: [512*batch_size, num_classes+1]
        # box_regression: [512*batch_size, 4*(num_classes+1)]
        ...
        if self.has_mask():
            ...
            if self.training:
                ...
                # proposals:[batch_size, 512, 4]
                num_images = len(proposals)
                mask_proposals = []
                pos_matched_idxs = []
                for img_id in range(num_images):
                    pos = torch.where(labels[img_id] > 0)[0]  # 寻找对应gt类别大于0，即正样本
                    mask_proposals.append(proposals[img_id][pos])  # [nums_pos, 4]
                    pos_matched_idxs.append(matched_idxs[img_id][pos])
            else:
                pos_matched_idxs = None

            # Mask分支:
            # 训练时mask_proposals由RPN提供，即proposals(只包含正样本)
            # 预测时mask_proposals由Fast R-CNN提供，二次修正后
            mask_features = self.mask_roi_pool(features, mask_proposals, image_shapes)
            mask_features = self.mask_head(mask_features)
            mask_logits = self.mask_predictor(mask_features)  # [all_nums_proposals, num_classes, 28, 28]
            ...
            if self.training:
                ...
                gt_masks = [t["masks"] for t in targets]  # [nums_instance, resized_H, resized_W]*batch_size
                gt_labels = [t["labels"] for t in targets]
                rcnn_loss_mask = maskrcnn_loss(mask_logits, mask_proposals, gt_masks, gt_labels, pos_matched_idxs)
                loss_mask = {"loss_mask": rcnn_loss_mask}
            else:
                labels = [r["labels"] for r in result]  # []*batch_size
                mask_probs = maskrcnn_inference(mask_logits, labels)  # [nums_mask_per_image, 1, 28, 28]*batch_size
