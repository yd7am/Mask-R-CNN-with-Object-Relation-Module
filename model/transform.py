...
def _resize_image_and_masks(image: Tensor,
    ...
    # size和scale_factor二者选其一？若fixed_size为None，则为等比例缩放：图像长宽比不变
    ...
    # 缩放完成后图片大小最长边被限制在[min_size,max_size]=[800,1333]中，不同图像大小不一样

    if target is None:  # target为空，即val模式
        return image, target

    # [n_polys, H, W] -> [1, n_polys, H, W]  mask[:, None] == mask[None]
    if "masks" in target:
        mask = target["masks"]
...
def resize_boxes(boxes, original_size, new_size):
    ...
    ]  # 若fixed_size=None，实际上长宽ratios都一样，等比例缩放
    ratios_height, ratios_width = ratios
    # Removes a tensor dimension, boxes [nums_coco_instance, 4]
    # Returns a tuple of all slices along a given dimension, already without it.
    xmin, ymin, xmax, ymax = boxes.unbind(1)  # 取到boxes矩阵每一列，其实直接对矩阵操作就可以
...
def paste_mask_in_image(mask, box, im_h, im_w):
    ...
    # mask: [30, 30], box: [4]原图, im_h,im_w: 原图尺寸
...
def paste_masks_in_image(masks, boxes, img_shape, padding=1):
    ...
    # masks：[num_obj, 1, 28, 28]，boxes是原图尺寸的[n_polys, 4]，img_shape是原图尺寸
    # pytorch官方说对mask进行expand能够略微提升mAP
    # refer to: https://github.com/pytorch/vision/issues/5845
    masks, scale = expand_masks(masks, padding=padding)  # [num_obj, 1, 30, 30]
    ...
    res = [paste_mask_in_image(m[0], b, im_h, im_w) for m, b in zip(masks, boxes)]  # [num_obj, H, W]
    if len(res) > 0:
...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def __init__(self,
        ...
        self.fixed_size = fixed_size  # 若不指定则为等比例缩放至[min_size,max_size]中
    ...
    def resize(self, image, target):
        ...
        if self.training:
            size = float(self.min_size)  # 指定输入图片的最小边长
        else:
        ...
        bbox = target["boxes"]  # [num_coco_targets, 4]
        # 根据图像的缩放比例来缩放bbox
    ...
    def max_by_axis(self, the_list):
        ...
        maxes = the_list[0]  # [3, img1_height, img1_width]
    ...
    def batch_images(self, images, size_divisible=32):
        # type: (List[Tensor], int) -> Tensor
        """
        将一批图像打包成一个batch返回（注意batch中每个tensor的shape是相同的，做了padding）
        Args:
            images: 输入的一批图片
        ...
        stride = float(size_divisible)  # 32
        ...
        batch_shape = [len(images)] + max_size  # [batch_size] + [3, max_h, max_w] = [batch_size, 3, max_h, max_w]

        # 创建shape为batch_shape且值全部为0的tensor
        batched_imgs = images[0].new_full(batch_shape, 0)  # Tensor下的方法
        for img, pad_img in zip(images, batched_imgs):
            # 将输入images中的每张图片复制到新的batched_imgs的每张图片中，对齐左上角，保证bboxes的坐标、mask掩膜不变
    ...
    def postprocess(self,
        ...
        for i, (pred, im_s, o_im_s) in enumerate(zip(result, image_shapes, original_image_sizes)):  # 解batch
            boxes = pred["boxes"]
            boxes = resize_boxes(boxes, im_s, o_im_s)  # 将bboxes缩放回原图像尺度上
            result[i]["boxes"] = boxes  # [n_polys, 4]
            if "masks" in pred:
                masks = pred["masks"]  # [num_obj, 1, 28, 28]
                # 将mask映射回原图尺度
    ...
    def forward(self,
        ...
        for i in range(len(images)):  # 解batch
            image = images[i]
            ...
            image, target_index = self.resize(image, target_index)  # 对图像和对应的bboxes、mask缩放到指定范围
            images[i] = image
        ...
        image_list = ImageList(images, image_sizes_list)  # images:打包后 image_size_list:原图像resize后尺寸 padding前 [(h1,w1),(h2,w2),...]
